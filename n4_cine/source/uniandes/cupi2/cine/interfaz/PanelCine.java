...
public class PanelCine extends JPanel
{
    ...
    public PanelCine( Silla[] lasSillas )
    {
        ...
        for( int i = 0; i < sillas.length; i++ )
        {
            ...
            {
                etiquetaSillas[ i ] = new JLabel( 
                		Character.toString( silla.darFila( ) ) + "0" + 
                				Integer.toString( silla.darNumero( ) ) );
            }
            else
            {
                etiquetaSillas[ i ] = new JLabel( 
                		Character.toString( silla.darFila( ) ) + 
                			Integer.toString( silla.darNumero( ) ) );
            }
            etiquetaSillas[ i ].setBackground( darColorSilla( silla ) );
            etiquetaSillas[ i ].setFont( etiquetaSillas[ i ].getFont( ).
            		deriveFont( ( float )10 ) );
            ...
            //Ubica la silla
            GridBagConstraints posicion = new GridBagConstraints( numero, fila, 
        ...
        etiquetaPantalla.setPreferredSize( new Dimension( 
        		( Cine.SILLAS_POR_FILA - 4 ) * 22, 30 ) );
        GridBagConstraints posicion = new GridBagConstraints( 2, 0, 
        		( Cine.SILLAS_POR_FILA - 4 ), 1, 0, 0, 
        			GridBagConstraints.CENTER, GridBagConstraints.CENTER, 
        				new Insets( 5, 5, 50, 5 ), 0, 0 );
